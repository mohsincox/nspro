...
class UpsellProfileReportController extends Controller
{
    ...
    public function upsellReportFormExcel()
    {

        return view('upsell_crm_profile.report.form_excel');
    }

    public function upsellReportShowExcel(Request $request)
    {
        //return $request->all();
        ...
        // return UpsellCrm::with(['profile', 'profile.division', 'profile.district', 'profile.policeStation', 'brand', 'category', 'createdBy'])->whereBetween('created_at', [$startDate, $endDate])->get();

       
        Excel::create('UpsellProfile'.$startDateShow.$endDateShow, function($excel) use ($startDate,  $endDate, $type) {

            $excel->sheet('Sheet1', function($sheet) use ($startDate,  $endDate, $type) {

                $fnms = UpsellCrm::with(['profile', 'profile.division', 'profile.district', 'profile.policeStation', 'brand', 'category', 'createdBy'])->whereBetween('created_at', [$startDate, $endDate])->get();
                ...
                foreach($fnms as $crm) {
                    ...
                    $data =  array($crm->id, $crm->created_at, $crm->agent, $crm->phone_number, $consumer_name, $productDetail, $crm->total_price, $consumer_gender, $child1Age, $address, $division, $child2Age, $consumer_age, $crm->product_used, $crm->product, $crm->multi_product, $crm->call_remarks, $crm->verbatim, $district, $crm->interested_in_fnm, $crm->interested_in_crm, $policeStation, $profession, $crm->profile_id, $number_of_child, $total_family_member, $child3Age, $categoryName, $brandName);
                    array_push($arr, $data);
                }
                
                //set the titles
                $sheet->fromArray($arr,null,'A1',false,false)->prependRow(array(
                        'SL No', 'Date', 'Agent Name', 'Consumer Phone Number', 'Consumer Name', 'Product Detail', 'Total Price', 'Consumer Gender', 'Child1 Age', 'Address', 'Division', 'Child2 Age', 'Consumer Age', 'Product Used', 'Product', 'Multi Product', 'Call Remarks', 'Verbatim', 'District', 'Interested FNM', 'Interested CRM', 'Police Station', 'Profession', 'Consumer ID', 'Child No.', 'Family Member', 'Child3 Age', 'Category', 'Brand'
                    )
