...
class ReportSingleProfileController extends Controller
{
    ...
    public function reportSingleFormExcel()
    {
        ...
        return view('report_single_profile.form', compact('brandList', 'divisionList', 'consumerAge', 'gender', 'sec', 'actOrCmpName'));
    }
            ...
            Excel::create('AllProfile_from_'.$request->start_date.'_to_'.$request->end_date, function($excel) use ($type, $startDate, $endDate) {
                ...
                $excel->sheet('Sheet1', function($sheet) use ($type, $startDate, $endDate) {
                    ...
                    foreach($profiles as $profile) {
                        ...
                        $data =  array($profile->id, $profile->phone_number,  $profile->consumer_name, $profile->consumer_age, $profile->consumer_gender, $division, $district, $policeStation, $profile->address, $profile->alternative_phone_number, $profile->profession, $profile->sec, $profile->number_of_child, $profile->total_family_member, $child1Age, $child2Age, $child3Age, $brandName, $profile->product, $profile->prefered_brand, $profile->activity_campaign_name, $profile->agent, $profile->updated_at);
                        array_push($arr, $data);
                    }
                    
                    //set the titles
                    $sheet->fromArray($arr,null,'A1',false,false)->prependRow(array(
                            'Id', 'Con Phone Number', 'Con Name', 'Con Age', 'Con Gender', 'Division', 'District', 'Police Station', 'Address', 'Alt. Ph No', 'Profession', 'SEC', 'Child No.', 'Family Mem.', 'Child1 Age', 'Child2 Age', 'Child3 Age', 'Brand', 'Product', 'Prefered Brand', 'Act or Cmp Name', 'Agent', 'CreatedOrUpdated At'
                        )
        ...
        else if ( ($request->start_date != null) && ($request->end_date != null) && ($request->type != null) && ($request->division_id != null) && ($request->district_id != null) && ($request->police_station_id == null) && ($request->consumer_age == null) && ($request->from_year == null) && ($request->from_month == null) && ($request->to_year == null) && ($request->to_month == null) && ($request->consumer_gender == null) && ($request->activity_campaign_name == null) && ($request->brand_id == null) && ($request->sec == null) ) 
        {
            ...
            Excel::create('DistrictWise_from_'.$request->start_date.'_to_'.$request->end_date, function($excel) use ($districtId, $type, $startDate, $endDate) {
                ...
                $excel->sheet('Sheet1', function($sheet) use ($districtId, $type, $startDate, $endDate) {
                    ...
                    //set the titles
                    $sheet->fromArray($arr,null,'A1',false,false)->prependRow(array(
                            'Id', 'Con Phone Number', 'Con Name', 'Con Age', 'Con Gender', 'Division', 'District', 'Police Station', 'Address', 'Alt. Ph No', 'Profession', 'SEC', 'Child No.', 'Family Mem.', 'Child1 Age', 'Child2 Age', 'Child3 Age', 'Brand', 'Product', 'Prefered Brand', 'Act or Camp. Name', 'Agent', 'CreatedOrUpdated At'
                        )
